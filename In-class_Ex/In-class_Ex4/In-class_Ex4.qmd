...
-   R package for building OLS and performing diagnostics tests
    ...
    -   [**olsrr**](https://olsrr.rsquaredacademy.com/)
        ...
        Very good package for building regression model

-   R package for calibrating geographical weighted family of models

    -   [**GWmodel**](https://cran.r-project.org/web/packages/GWmodel/)

        can have geographically weighted summary statistics/PCA/discriminant analysis
...
There are erroneous polygon(s) in the map, ideally should use GIS package to fix but short fix is to use tmap_options(check.and.fix = TRUE)
...
Here we do not use the sf dataframe as there is already a geometry field, we use condo_resale as there is no geomtry field for correlation

```{r}
corrplot.mixed(cor(condo_resale[, 5:23]), diag = NULL, order = "AOE",lower = "number", upper = "ellipse",tl.pos = "lt", tl.cex = 0.5,number.cex=0.3,lower.col = "black")
```
...
Every 1 unit increase in AREA_SQM, the price increase by 12777.523,

Every 1 unit increase in AGE, price will drop by 24687.739.
...
The figure reveals that the residual of the multiple linear regression model (i.e. condo.mlr1) is resemble normal distribution. residual = predicted value - observed value
...
always use \` for variable name to be safe
...
There are two possible approaches can be uused to determine the stopping rule, they are: CV cross-validation approach and AIC corrected (AICc) approach. We define the stopping rule using ***approach*** argeement. longlat = FALSE because coordinates already projected, no longer is decimal degrees.
...
The result shows that the recommended bandwidth is 971.3398 metres. The bandwidth is in metres as the the EPSG code is 3414 which is in metres.
...
set.zoom.limits to ensure user cannot zoom too much in or too much out
...
The code chunks below is used to create an interactive point symbol map. use sync = true to ensure both map zooms and out in sync together.
...
> > > > > > > dd6f438525d409fbd2651268031103ea2b199af8
